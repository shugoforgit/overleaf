...
\begin{itemize}
  \item 编写合约代码实现任意NFT上架、下架、手续费提取等基础功能
  \item 编写合约代码实现NFT售卖,使用ReentrancyGuard防止重入,使用safeTransferFrom安全转移NFT
  \item 根据Staking Reward算法实现质押ETH奖励分配逻辑的Farm合约，合约中存储用户质押信息和流动池信息结构
  \item 使用OpenZeppelin的upgrades插件简化智能合约的部署和升级,安全的管理合约的升级
  \item 编写合约部署脚本保证合约的顺序部署和合约依赖地址设置,使用upgradeProxy升级合约
  \item 基于测试驱动开发编写单元测试确保合约稳定性和准确性
  \item 参与了python到go微服务的迁移重构
\end{itemize}
